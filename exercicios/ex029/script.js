...
function inLista(n, l) {
  //função que verifica se o número já foi inserido
...
function adicionar() {
  if (isNumero(num.value) && !inLista(num.value, valores)) {
    //determina os atributos reais das funçoes recursivas
    valores.push(Number(num.value)); //adiciona um elemento ao array valores
    let item = document.createElement("option");
    item.text = `Valor ${num.value} adicionado.`;
    lista.appendChild(item);
    resultado.innerHTML = ""; //zera o conteúdo da variavel resultado sempre que um outro número é adicionado
  } else {
